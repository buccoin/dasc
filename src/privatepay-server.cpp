...
void CPrivatePayServer::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if(fLiteMode) return; // ignore all DasCoin related functionality
...
/*
    ...
    allow endless transaction that would bloat DasCoin and make it unusable. To
    stop these kinds of attacks 1 in 10 successful transactions are charged. This
    adds up to a cost of 0.001 DASC per transaction on average.
*/
...
{
    if(fLiteMode) return; // disable all DasCoin specific functionality
    ...
    RenameThread("DasCoin-ps-server");
