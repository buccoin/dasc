...
void CActiveMasternode::ManageStateInitial()
{
    ...
    if(pwalletMain->GetBalance() < Params(CBaseChainParams::MAIN).GetConsensus().masternodeCount) {
        LogPrintf("CActiveMasternode::ManageStateInitial -- %s: Wallet balance is < 10000 DASC\n", GetStateString());
        return;
    }
