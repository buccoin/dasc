 ...
 *
 *   // network
 *   CDasCoinNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.DasCoin.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CDasCoinNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CDasCoinNetwork(UniValue objIn)
    {
...
// // can be under: DasCoinNetwork
// //   -- signature requirements : Key1(User)
// class CDasCoinNetworkVariable : public CGovernanceObject
// {
// private:
