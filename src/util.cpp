...
const char * const BITCOIN_CONF_FILENAME = "DasCoin.conf";
const char * const BITCOIN_PID_FILENAME = "DasCoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "DasCoin" is a composite category enabling all DasCoin-related debug output
            if(ptrCategory->count(string("DasCoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DasCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DasCoin
    // Mac: ~/Library/Application Support/DasCoin
    // Unix: ~/.DasCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DasCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DasCoin";
#else
    // Unix
    return pathRet / ".DasCoin";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty DasCoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override DasCoin.conf
