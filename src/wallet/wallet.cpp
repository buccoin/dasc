        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !(fMasterNode && pcoin->vout[i].nValue == Params(CBaseChainParams::MAIN).GetConsensus().masternodeCount);
                } else if(nCoinType == ONLY_NONDENOMINATED_NOT100000IFMN) {
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && fMasterNode) found = pcoin->vout[i].nValue != Params(CBaseChainParams::MAIN).GetConsensus().masternodeCount; // do not use Hot MN funds
                } else if(nCoinType == ONLY_100000) {
                    found = pcoin->vout[i].nValue == Params(CBaseChainParams::MAIN).GetConsensus().masternodeCount;
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, std::vector<COutput>& vCoinsRet, CAmount& nValueRet, int nPrivatePayRoundsMin, int nPrivatePayRoundsMax)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        //if(out.tx->vout[out.i].nValue == Params(CBaseChainParams::MAIN).GetConsensus().masternodeCount) continue;
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nValueToSelect, setCoins, nValueIn, coinControl, nCoinType, fUseInstaPay))
                {
                    if (nCoinType == ONLY_NOT100000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 DASC.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT100000IFMN) {
                        strFailReason = _("Unable to locate enough PrivatePay non-denominated funds for this transaction that are not equal 1000 DASC.");
                    ...
                    if (fUseInstaPay) {
                        if (nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTAPAY_MAX_VALUE)*COIN) {
                            strFailReason += " " + strprintf(_("InstaPay doesn't support sending values that high yet. Transactions are currently limited to %1 DASC."), sporkManager.GetSporkValue(SPORK_5_INSTAPAY_MAX_VALUE));
                        } else {
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-DasCoin-address
                        CScript scriptChange;
