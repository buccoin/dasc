...
#if defined(NDEBUG)
# error "DasCoin Core cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "DasCoin Signed Message:\n";
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight == 1) {
        nSubsidyBase = 500000000*COIN;
    } else if (nPrevHeight < 4166667) {
...
{
    CAmount ret = 100*COIN;//blockValue/2; // start at 40%
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // DASC : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if(pindex->pprev->nHeight == 1){
        blockReward=blockReward+3000000*COIN;
    }
    std::string strError = "";
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(DASC): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(DASC): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // DASC : CHECK TRANSACTIONS FOR INSTAPAY
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTAPAY_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instapay.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(DASC): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(DASC): spork is off, skipping transaction locking checks\n");
    }
